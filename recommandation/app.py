...
def load_aptitude_questions(file_path):
    ...
    with open(file_path, 'r', newline='', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            ...
            # Normalize answer: support index (1..4) or exact option text
            raw_answer = str(row['answer']).strip()
            if raw_answer.isdigit():
                answer_idx = raw_answer
            else:
                try:
                    answer_idx = str(options.index(raw_answer) + 1)
                except ValueError:
                    # Fallback to first option if data mismatch
                    answer_idx = '1'
            question = {
                'id': row['id'],
                'question': row['question'],
                'options': options,
                # store answer as index (1..4) to match template loop.index
                'answer': answer_idx
            }
...
def aptitude_test():
    if request.method == 'GET':
        ...
        for question in aptitude_questions:
            ...
            # user_answer is a string of the selected option index (1..4)
            if user_answer and user_answer.strip() == str(question['answer']).strip():
                score += 1
